/**
 * This is 99% of the code is from the tinykeys library (small adjustments)
 * NPM: https://www.npmjs.com/package/tinykeys
...
/**
 * Keybinding should not execute inside of input or textarea
 */
let DEFAULT_IGNOREFOCUS = false;
...
export function createKeybindingsHandler(
  ...
  return (event) => {
    ...
    // Ensure and stop any event if ignoreFocus is enabled
    // if user is focused on input or textarea
    const ignoreFocus = options.ignoreFocus || DEFAULT_IGNOREFOCUS;
    const nodeName = (event.target as Element).nodeName.toLowerCase();
    if (
      !ignoreFocus && (nodeName === 'input' ||
      nodeName === 'textarea')
    ){
      return;
    }
